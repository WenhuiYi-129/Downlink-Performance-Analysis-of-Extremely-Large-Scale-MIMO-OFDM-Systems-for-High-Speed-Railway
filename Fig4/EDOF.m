...
EDOF1_sUPAclosed=zeros(length(LT1),lenxr);
EDOF1_sUPAunclosed=zeros(length(LT1),lenxr);
EDOF1_sULAclosed=zeros(length(LT1),lenxr);
EDOF1_sULAunclosed=zeros(length(LT1),lenxr);
for j=1:length(LT1)
...
Nr_X=M; Nr_Y=M;
Ns_X=M; Ns_Y=M;
    RecSpacing=LT1(j)*sqrt(2)/2/M;
    TraSpacing=LT1(j)*sqrt(2)/2/M;
RecLength.L_x=LT1(j)*sqrt(2)/2;RecLength.L_y=LT1(j)*sqrt(2)/2;
RecLength.distance=[xr,-6,2];
...
%         noise=sqrt(var_noise/2)*(randn(UsersNum*RecNumNr,1)+1i*randn(UsersNum*RecNumNr,1));
        ...
        ChannelHa=M^2*sqrt(channelGain)*ChannelSigma.*ChannelRandom/sum(ChannelSigma(:));
...
l=2;
s=2;
EDOF1_sUPAclosed(j,i)=sUPAclosed(Ns_X,Ns_Y,Nr_X,Nr_Y,TraLength.L_x,TraLength.L_y,RecLength.L_x,RecLength.L_y,fc,c,RecLength.distance(1),RecLength.distance(2),RecLength.distance(3),u,l,s,f,L,Ts,Channel,laisi_k);
...
Ns_X=1; Ns_Y=M^2;
spacing_x=lambda/2;
    RecSpacing=LT1(j)/M^2;
    TraSpacing=LT1(j)/M^2;
...
%         Channel=1/sqrt(UsersNum*RecNumNr*TraNumNs)*RecResVector*ChannelHa*TraResVector;
 ...
 l=2;
s=2;
EDOF1_sULAclosed(j,i)=sULAclosed(Ns_Y,Nr_Y,TraLength.L_y,RecLength.L_y,fc,c,RecLength.distance(1),RecLength.distance(2),RecLength.distance(3),u,l,s,f,L,Ts,Channel,laisi_k);
...
figure(1);
h1=plot(xr1./lambda,EDOF1_sUPAclosed(1,:),'o','LineWidth',1.5);
hold on ;
h2=plot(xr1./lambda,EDOF1_sUPAunclosed(1,:),'--','LineWidth',2);
hold on ;

h3=plot(xr1./lambda,EDOF1_sUPAclosed(2,:),'o','LineWidth',1.5);
hold on ;
h4=plot(xr1./lambda,EDOF1_sUPAunclosed(2,:),'--','LineWidth',2);
hold on ;
h5=plot(xr1./lambda,EDOF1_sULAclosed(1,:),'o','LineWidth',1.5);
hold on ;
h6=plot(xr1./lambda,EDOF1_sULAunclosed(1,:),'--','LineWidth',2);
hold on ;

h7=plot(xr1./lambda,EDOF1_sULAclosed(2,:),'o','LineWidth',1.5);
hold on ;
h8=plot(xr1./lambda,EDOF1_sULAunclosed(2,:),'--','LineWidth',2);
hold on ;
xlabel('Y-axis Distance $|y_r|$ $(\lambda)$','Interpreter','Latex');
ylabel('EDoF','Interpreter','Latex');
legend([h2 h4 h6 h8 h1],{'UPA,$D=min\lambda$','UPA,$D=max\lambda$','ULA,$D=min\lambda$','UPA,$D=max\lambda$','Analytical'},'Interpreter','Latex','Location','Northwest')
set(gca,'FontSize',12);
grid on;
toc;
